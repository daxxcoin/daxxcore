...
// Service implements an Daxxcoin netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *eth.Daxxcoin      // Full Daxxcoin service if monitoring a full node
	les    *les.LightDaxxcoin // Light Daxxcoin service if monitoring a light node
...
func New(url string, ethServ *eth.Daxxcoin, lesServ *les.LightDaxxcoin) (*Service, error) {
