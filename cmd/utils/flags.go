...
// Package utils contains internal helper functions for daxxcorecommands.
package utils
...
import (
	...
	"github.com/daxxcoin/daxxcore/metrics"
	...
	whisper "github.com/daxxcoin/daxxcore/whisper/whisperv2"
...
var (
	...
	DaxxcoinbaseFlag = cli.StringFlag{
		Name:  "daxxcoinbase",
...
// MakeDaxxcoinbase retrieves the daxxcoinbase either from the directly specified
// command line flags or from the keystore if CLI indexed.
func MakeDaxxcoinbase(ks *keystore.KeyStore, ctx *cli.Context) common.Address {
	accounts := ks.Accounts()
	if !ctx.GlobalIsSet(DaxxcoinbaseFlag.Name) && len(accounts) == 0 {
		glog.V(logger.Error).Infoln("WARNING: No daxxcoinbase set and no accounts found as default")
		return common.Address{}
	}
	daxxcoinbase := ctx.GlobalString(DaxxcoinbaseFlag.Name)
	if daxxcoinbase == "" {
		return common.Address{}
	}
	// If the specified daxxcoinbase is a valid address, return it
	account, err := MakeAddress(ks, daxxcoinbase)
	if err != nil {
		Fatalf("Option %q: %v", DaxxcoinbaseFlag.Name, err)
	}
...
func MakeNode(ctx *cli.Context, name, gitCommit string) *node.Node {
	...
	if ctx.GlobalBool(DevModeFlag.Name) {
		if !ctx.GlobalIsSet(DataDirFlag.Name) {
			config.DataDir = filepath.Join(os.TempDir(), "/daxxcoin_dev_mode")
		}
...
// RegisterEthService configures eth.Daxxcoin from command line flags and adds it to the
// given node.
func RegisterEthService(ctx *cli.Context, stack *node.Node, extra []byte) {
	...
	ethConf := &eth.Config{
		Daxxcoinbase:               MakeDaxxcoinbase(ks, ctx),
	...
	// Override any global options pertaining to the Daxxcoin protocol
	...
	if ethConf.LightMode {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			Fatalf("Failed to register the Daxxcoin light node service: %v", err)
		}
	} else {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			...
			Fatalf("Failed to register the Daxxcoin full node service: %v", err)
		}
	}
}
...
// RegisterEthStatsService configures the Daxxcoin Stats daemon and adds it to
// th egiven node.
func RegisterEthStatsService(stack *node.Node, url string) {
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		// Retrieve both eth and les services
		var ethServ *eth.Daxxcoin
		ctx.Service(&ethServ)
		...
		Fatalf("Failed to register the Daxxcoin Stats service: %v", err)
	}
}
