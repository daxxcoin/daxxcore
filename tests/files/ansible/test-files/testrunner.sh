...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-daxxcoin/build/test/createRandomStateTest\" daxxcoin/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pydaxxcoin
	OUTPUT_PYTHON="$(docker run --rm daxxcoin/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm daxxcoin/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm daxxcoin/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
