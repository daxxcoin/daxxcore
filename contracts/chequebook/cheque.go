...
// Package chequebook package wraps the 'chequebook' Daxxcoin smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in daxxcoin; (auto)cashing cheques in daxxcoin
// as well as (auto)depositing daxxcointo the chequebook contract.
package chequebook
...
import (
	...
	"github.com/daxxcoin/daxxcore/contracts/chequebook/contract"
	...
	"github.com/daxxcoin/daxxcore/swarm/services/swap/swap"
...
// * depositing daxxcointo the chequebook
// * watching incoming daxxcoin
...
func (self *Chequebook) deposit(amount *big.Int) (string, error) {
	...
	if err != nil {
		glog.V(logger.Warn).Infof("error depositing %d deito chequebook (%s, balance: %v, target: %v): %v", amount, self.contractAddr.Hex(), self.balance, self.buffer, err)
		return "", err
	}
	// assume that transaction is actually successful, we add the amount to balance right away
	self.balance.Add(self.balance, amount)
	glog.V(logger.Detail).Infof("deposited %d deito chequebook (%s, balance: %v, target: %v)", amount, self.contractAddr.Hex(), self.balance, self.buffer)
...
func (self *Inbox) Receive(promise swap.Promise) (*big.Int, error) {
	...
	if err == nil {
		...
		glog.V(logger.Detail).Infof("received cheque of %v deiin inbox (%s, uncashed: %v)", amount, self.contract.Hex(), uncashed)
	}
...
// Cash cashes the cheque by sending an Daxxcoin transaction.
