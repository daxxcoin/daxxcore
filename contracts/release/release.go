...
type Config struct {
	Oracle common.Address // Daxxcoin address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Daxxcoin service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var daxxcoin *eth.Daxxcoin
	if err := ctx.Service(&daxxcoin); err == nil {
		apiBackend = daxxcoin.ApiBackend
	} else {
		var daxxcoin *les.LightDaxxcoin
		if err := ctx.Service(&daxxcoin); err == nil {
			apiBackend = daxxcoin.ApiBackend
		} else {
			return nil, err
	...
	for {
		...
		case <-timer.C:
			...
			if version.Major > r.config.Major ||
				...
				howtofix := fmt.Sprintf("Please check https://github.com/daxxcoin/daxxcore/releases for new releases")
