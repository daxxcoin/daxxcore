...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.daxxcoin", new NodeConfig());
			node.start();
			...
			DaxxcoinClient ec = node.getDaxxcoinClient();
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/daxxcoin/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.daxxcoin", "github.com/daxxcoin/daxxcore/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "daxxcoin", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
