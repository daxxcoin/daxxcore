...
import (
	...
	"github.com/daxxnucleus/daxxhash"
	"github.com/daxxcoin/daxxcore/accounts"
	"github.com/daxxcoin/daxxcore/accounts/keystore"
...
// PublicDaxxcoinAPI provides an API to access Daxxcoin related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicDaxxcoinAPI struct {
	b Backend
}

// NewPublicDaxxcoinAPI creates a new Daxxcoin protocol API.
func NewPublicDaxxcoinAPI(b Backend) *PublicDaxxcoinAPI {
	return &PublicDaxxcoinAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicDaxxcoinAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Daxxcoin protocol version this node supports
func (s *PublicDaxxcoinAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicDaxxcoinAPI) Syncing() (interface{}, error) {
...
func (s *PublicTxPoolAPI) Inspect() map[string]map[string]map[string]string {
	...
	var format = func(tx *types.Transaction) string {
		if to := tx.To(); to != nil {
			return fmt.Sprintf("%s: %v dei+ %v × %v gas", tx.To().Hex(), tx.Value(), tx.Gas(), tx.GasPrice())
		}
		return fmt.Sprintf("contract creation: %v dei+ %v × %v gas", tx.Value(), tx.Gas(), tx.GasPrice())
	}
	// Flatten the pending transactions
	for account, txs := range pending {
...
//   keccak256("\x19Daxxcoin Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Daxxcoin Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Daxxcoin ECDSA signature for:
// keccack256("\x19Daxxcoin Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/daxxcoin/daxxcore/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Daxxcoin Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/daxxcoin/daxxcore/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Daxxcoin signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Daxxcoin blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Daxxcoin blockchain API.
...
// GetBalance returns the amount of deifor the given address in the state of the
...
//
// https://github.com/daxxcoin/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of Daxxcoin APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of Daxxcoin APIs exposed over the private
// debugging endpoint.
...
// Version returns the current daxxcoin protocol version.
